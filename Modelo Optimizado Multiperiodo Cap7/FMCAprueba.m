...
vd = vdopt;
%vd = [0.3841  0.2462  0.2978  0.0719  0.0714  0.9126  0.0160  0.2705  0.2184  0.1842  0.0127  0.3142];
...
%% CONTROL DE CO2
%CO2eq(1) = 551;        %ppm 
CO2eq(1) = 522;        %ppm  Datos de la NOAA para 2020 https://gml.noaa.gov/aggi/
CO2(1) = CO2eq(1)*.75; %ppm emision en 2019


% *** 490 to 1260 ppm (CO2) para 2100 https://archive.ipcc.ch/ipccreports/tar/vol4/index.php?idp=9#:~:text=For%20the%20six%20illustrative%20SRES,the%20period%201990%20to%202100.
% mas o menos 650 a 1680 (CO2eq) usando .75 de CO2 a CO2eq
...
dCons = 174.37; % ppm
%dEmis = 2400 ; %Gton
dEmis = 2400*0.29 ; %Gton Para considerar cuantas Gt de GHG se han emitido y permanecido en la atmósfera

fr =  dCons/dEmis;

%% CONTROL DE TEMPERATURA
atemp(1) = -0.21;  %Anomalía inicial
...
% RP si temp_PL == 1  
gRPP1am = gRPP1a *hg;       %0%-60%
gRPP1bm = gRPP1b *hg;       %40%-100% (40% de areas cultivadas no disponibles para P1a)
gRPP2m  = gRPP2 *hg;
% P1a
...
wpob_mund = (59-10.94443)/(pob_mund(1)*12); %Emisi Anuales - AFOLU - El % de desechos ***ENTRE 12 PARA HACERLO MENSUAL
...
%se agrega las variables multiperiodo dentro del sicho if linea 319
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  START LOOP   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 for i=1 : time
   ...
   gRPP2 = gRPP2m*g_CO2_Temp([dCO2(i),temp(i)]');
...
%%%%%% CO2eq calculado a partir de la Produccion alimentarias%%%%%   
    CO2eq(i+1) = CO2eq(i) + h * fr * ((waRP*RP(i)) + (waP2*P2(i)) + (CO2eqP1a + CO2eqH1r + CO2eqH1m + CO2eqP1b + CO2eqP2 + CO2eqHH)*12 + (wpob_mund*pob_mund(i)) + (wH2*H2(i)) + (wC1*C1(i))); %*** AFOLU se multiplica por 12 ya que son emisiónes por flujo mensual y las demas son anuales
    CO2(i+1) = CO2eq(i)*0.75;
...
%% %%% POPULATION %%%%% 
    ...
    if incre_HH == 1
        ...
        %HR(i)= -0.0000953467*((IMCpc(i))^3) + (0.0126729*(IMCpc(i))^2) - 0.477231*IMCpc(i) + 6.46637;  %curva inversa de muerte por mala alimentación
                
        %****************************************************************************************
                
        birth_rate(i) = (36.4499771662*exp(-0.0009862446*(i+732)))/(1000*52); %Ajuste exponencial en funcion del tiempo
        
        mortality_rate(i) =( -2.3569365994*log(i+732) + 23.2130036155)/(1000*52);
        
        %mHH = 0.0038*(IMCpc(i)*IMCpc(i)) - 0.2108*IMCpc(i) + 3.5436;
        %mHH = 0.00542921*(IMCpc(i)^2) - 0.30119112*IMCpc(i) + 4.06224860;
       
        %Pr
        mHH = 0.00005368*(IMCpc(i)^4) - 0.00627589*(IMCpc(i)^3) + 0.27424943*(IMCpc(i)^2) - 5.29297998*IMCpc(i) + 37.92871035;
       
        mHH = m_IMC([IMCpc(i)]); %potencializacion de tasa de mortalidad actual por IMC
        
     
        %pob_mund(i+1)= (HH(i) + (HH(i)*birth_rate(i)) - (HH(i)*mortality_rate(i)))/ppHH;
        %pob_mund(i+1) = pob_mund(i) + birth_rate(i)*pob_mund(i) - mortality_rate(i)*pob_mund(i);
        pob_mund(i+1) = pob_mund(i) + birth_rate(i)*pob_mund(i) - mortality_rate(i)*pob_mund(i)*mHH;
