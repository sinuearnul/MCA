...
%% Resultados Modelo de Capítulo 4 y 5
% Exclusivamente para la Grafica de Atemp, se agrega un desplazamiento de 0.3 
%en la Ec. de atemp. (linea 295)
AjusteGraficaAtemp = 0.0;

%% INPUTS

temp_PL = 1;  %Afectacion de Temp en plantas 0=NO  1=SI
incre_HH = 1;  %Incremento poblacionala 0=NO  1=SI (tasa de natalidad y mortandad)  2=SI (increm poblacional)
time = 960;    %Tiempo en meses

 %% CONTROL DE TIEMPO
tp=12; % meses por anio (PARA GRAFICAR)
h=1/12; %tamano de paso ecuaciones 
hg=1/12; %tamano de paso gradientes
hw=1/12;
yeara(1) = 2021;
...
%{ 
PARA ANALISIS ORDEN DE MAYOR A MENOR APORTACION
wHH=	0.0075899720
wH1m=	0.0011073470
wH1r=	0.0004678270
wP1a=	0.0001352800
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  START LOOP   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 for i=1 : time
           ...
           %PLANTAS
   if temp_PL == 1  
%   gRPP1a=gRPP1am*exp(-((temp(i)-tempo)^2)/100);                  
%   gRPP1b=gRPP1bm*exp(-((temp(i)-tempo)^2)/100);
%   gRPP2=gRPP2m*exp(-((temp(i)-tempoc^2)/100);
...
%%%%% CO2eq %%%%%  
CO2eqasd(1) = 0;
    CO2eq(i+1) = CO2eq(i) + (wRP*RP(1)) + (wP1a*P1a(i)) + (wP1b*P1b(i)) + (wP2*P2(i)) + (wH1r*H1r(i)) + (wH1m*H1m(i)) + (wH2*H2(i)) + (wpob_mund*pob_mund(i)) + (wC1*C1(i));
...
%%%%% TEMPERATURA %%%%%   
      ...
      atemp(i+1)=0.0000213866*(CO2eq(i)^2) - 0.0136962886*CO2eq(i) + 2.2936139065 + AjusteGraficaAtemp; %se suma 0.3 para desplazar la grafica de Atemp hacia arriba
...
%%%%% POPULATION %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%Revisar 
    ...
    pob_mund(i+1)= pob_mund(i) + pob_mund(i)*birth_rate(i) - mortality_rate(i)*pob_mund(i);
    end
